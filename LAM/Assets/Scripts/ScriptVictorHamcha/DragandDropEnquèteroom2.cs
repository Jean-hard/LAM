...
public class DragandDropEnquèteroom2 : MonoBehaviour
{
    ...
    private Vector2 initialposition;//valeur de la position initial de la pièce 
    private bool droped;//vérifie si la pièce a été drop dans un des slots 
    public List<GameObject> goodpiece = new List<GameObject>();//es-ce que la pièce fait partie des bonnes pièce 
    public List<bool> piececorrectpos = new List<bool>();//vérirfie si la piece es a la bonne position 
    ...
    void Update()
    {
        if (selected)// si la pièce est séléctioné 
        {
        ...
        for (int i = 0; i <= 3; i++)// vérifie pour toutes les pièce de la liste  
        {
            ...
            if (distgoodnumber == 0)//si la pièce est dans le bon slot alors  
            {
                ...
                piececorrectpos[i] = true;// le booléen le vérifient devient true 
        ...
        if (Input.GetKeyUp(KeyCode.Mouse0))//si on lève le click de la souris 
        {
            ...
            if (!droped && selected)// si la pièce séléctioné n'a pas été drop dans un slot alors... 
            {
                transform.position = initialposition;//la pièce reprend sa positon initial 
            }
