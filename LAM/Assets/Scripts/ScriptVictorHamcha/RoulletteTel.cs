    ...
    void Update()
    {
        ...
        if (Input.GetKeyUp(KeyCode.Mouse0))// si je lache le clic gauche de la souris la pièce est déléctioné
        {
            ...
            if (selected)// si c'etait le numéro selectioné
            {
                ...
                if (telephonemanager.numtel.Count <= 5&& distance<=0.2)// si le nombre de numéro fait est inferieur au nombre de numéro demandé et que la distance entre l'ancre et le numéro séléctioné est inferieur à 0.2 alor
                {
